...
from iwillwin.data_utils import bm25
...
class FeatureCreator(object):
    ...
    def __init__(self, train_df, test_df, data_loader, normalization=True):
        self.train_df = train_df
        self.test_df = test_df

        self.data_loader = data_loader
        self.stopwords = set() # unused
        ...
        docs_raw = self.train_df['spn_1'].values.tolist() + self.train_df['spn_2'].values.tolist() + \
                   self.test_df['spn_1'].values.tolist() + self.test_df['spn_2'].values.tolist()

        doc_set = set()
        doc_ids = []
        for ids, doc in enumerate(docs_raw):
            if doc in doc_set:
                continue
            else:
                doc_ids.append(ids)
        ...
        docs_raw = np.array(docs_raw)[doc_ids].tolist()
        docs = self.train_df['splited_spn_1'].values.tolist() + self.train_df['splited_spn_2'].values.tolist() + \
               self.test_df['splited_spn_1'].values.tolist() + self.test_df['splited_spn_2'].values.tolist()
        docs = np.array(docs)[doc_ids].tolist()
        self.sentences = docs
...
class CharFeatureCreator(FeatureCreator):
    ...
    def __init__(self, train_df, test_df, data_loader, normalization=True):
        ...
        self.train_df['splited_spn_1'] = self.train_df['spn_1'].apply(lambda v: [e for e in v if e !=' '])
        self.train_df['splited_spn_2'] = self.train_df['spn_2'].apply(lambda v: [e for e in v if e !=' '])

        self.test_df['splited_spn_1'] = self.test_df['spn_1'].apply(lambda v: [e for e in v if e !=' '])
        self.test_df['splited_spn_2'] = self.test_df['spn_2'].apply(lambda v: [e for e in v if e !=' '])
